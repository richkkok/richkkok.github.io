...
입력은 금액→소비처→저장이 기본이며 소비처 분류를 학습합니다. 결제경로와 실제 카드/계좌는 동일 거래의 속성입니다. 동일 입력은 경고와 확인을 거치며 반복 저장은 ID로 방어합니다. 카드사 원본 엑셀은 본인/가족 소계 구간과 실제 이용행을 자동 분리하고, 이름이 포함된 명세서는 현재 구성원과 이름으로 연결합니다. 우리카드 텍스트형 PDF는 고정 버전 PDF.js로 브라우저 안에서 텍스트 좌표를 읽어 카드 구간·소계·거래행을 분리하며, 일반 카드 PDF는 보수적인 표 형식 휴리스틱이 충분히 맞을 때만 자동 인식합니다. 소계·합계는 저장하지 않고 취소/음수 금액은 환불로 처리합니다. 우리카드 PDF는 할인 전 이용금액(grossAmount), 실제 납부금액(payableAmount), 카드혜택(cardBenefitAmount)을 함께 보존하고 예산·분석에는 실제 납부금액을 사용합니다. 여러 파일은 순서대로 자동분류 또는 열 연결→미리보기→중복 후보 확인 후 저장합니다. 자동분류/고정비 추론은 수정할 수 있습니다.
...
자체 호스팅 아이콘·스프레드시트 파서와 전체 앱 파일의 내용 해시로 캐시를 교체합니다. PDF.js는 버전이 고정된 jsDelivr 모듈만 허용하며 원본 금융파일은 해당 모듈 또는 서버로 전송하지 않습니다. 설치가 실패하면 부분 캐시를 폐기합니다. 새 버전은 입력창이 닫힌 뒤 사용자가 적용하며 IndexedDB 기록은 유지됩니다. manifest의 기존 id/scope를 유지해 설치 식별자를 바꾸지 않습니다.
