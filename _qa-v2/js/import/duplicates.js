...
export function possibleDuplicates(incoming, existing) {
  ...
  return incoming.flatMap(t=>{
    const rows=groups.get([t.owner,t.date,t.amount,t.direction].join('|'))||[];
    const match=rows.find(x=>x.sourceId!==t.sourceId&&(keyText(x.merchantNormalized)===keyText(t.merchantNormalized)||((/페이|pay/i.test(x.paymentMethod+' '+t.paymentMethod+' '+x.merchantRaw+' '+t.merchantRaw))&&x.datetime===t.datetime)));
