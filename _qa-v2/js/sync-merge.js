...
export function mergeStates(base, local, remote, choices = {}) {
  ...
  function conflict(b, l, r, path) {
    ...
    return clone(choices[path] === "remote" ? r : l);
  }
    ...
    if (Array.isArray(l) && Array.isArray(r)) {
      ...
      if (all.every((x) => object(x) && typeof x.id === "string")) {
        const map = (a) => new Map((a || []).map((x) => [x.id, x])),
          ...
          lm = map(l),
          rm = map(r);
        return [...new Set([...bm.keys(), ...lm.keys(), ...rm.keys()])]
          .map((id) =>
            walk(bm.get(id), lm.get(id), rm.get(id), `${path}[${id}]`),
          )
...
export function mergeValue(base, local, remote) {
  ...
  if (result.conflicts.length) {
    const error = Error(
      "같은 기록이 두 기기에서 변경됐어요. 가족·설정에서 동기화 충돌을 확인해 주세요.",
    );
