...
export const dayDiff = (a,b) => Math.round((Date.parse(b+'T12:00:00Z')-Date.parse(a+'T12:00:00Z'))/86400000);
...
export const transactionPeriod = (tx,state) => tx.operatingMonth || periodKey(tx.date,startDay(state));
export const inPeriod = (tx,state,month) => transactionPeriod(tx,state) === month;
export const periodLabel = p => `${Number(p.start.slice(5,7))}.${Number(p.start.slice(8))} – ${Number(p.end.slice(5,7))}.${Number(p.end.slice(8))}`;
export const periodDates = p => Array.from({length:p.days},(_,i)=>addDays(p.start,i));
