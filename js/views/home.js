...
export function homeView(state, month) {
  ...
  const kpis = [
    ...
    {
      label: "예상 잔액",
      value: b.expectedBalance,
      hint: recorded ? "현재 소비속도 추정" : "설정 기준",
    },
  ];
  ...
  <section class="card household-card">${sectionTitle("함께 정한 생활비", "공동생활비·용돈은 이번 달만 따로 바꿀 수 있어요", button("이번 달 수정", "edit-budget", "text"))}<div class="budget-hero"><span>남은 생활비</span><strong class="${b.plan.sharedBudget - b.shared < 0 ? "negative" : ""}">${won(b.plan.sharedBudget - b.shared)}</strong></div>${progress(b.shared, b.plan.sharedBudget)}<div class="between small muted"><span>${won(b.shared)} 사용</span><span>목표 ${shortWon(b.plan.sharedBudget)}</span></div><div class="personal-budgets">${["p1", "p2"].map((p) => `<div><span class="avatar">${e(state.settings.members[p].slice(0, 1))}</span><div><strong>${e(state.settings.members[p])} 용돈</strong><small>${shortWon(b.personal[p])} / ${shortWon(b.plan.personalBudgets[p])}</small></div><span class="personal-left">${shortWon(b.plan.personalBudgets[p] - b.personal[p])}<small>남음</small></span></div>`).join("")}</div><p class="privacy-note">${icon("lock")}개인 가맹점은 ${state.settings.privacy ? "숨겨져" : "표시되어"} 있어요</p></section>
  <section class="kpi-strip">${kpis
    .map((item) => {
      const content = `<span>${e(item.label)}</span><strong class="${item.value < 0 ? "negative" : ""}">${shortWon(item.value)}</strong><small>${e(item.hint)}</small>`;
      return item.action
        ? `<button type="button" class="kpi-cell kpi-action" data-action="${e(item.action)}" aria-label="${e(item.aria)}">${content}</button>`
        : `<div class="kpi-cell">${content}</div>`;
    })
    .join("")}</section>
  ...
  <section class="card recurring-summary">${sectionTitle("다가오는 고정·반복비", "항목을 누르면 바로 수정할 수 있어요", button("관리", "recurring-list", "text"))}${
    b.recurring
      .filter((r) => r.outstanding > 0)
      .slice(0, 4)
      .map(
        (r) =>
          `<button type="button" class="schedule-row schedule-row-button" data-recurring="${e(r.id)}"><span class="schedule-day">${Number(r.date.slice(8))}<small>일</small></span><span class="schedule-copy"><strong>${e(r.name)}</strong><small>${e(r.paymentMethod || "결제수단 미정")}</small></span><strong>${won(r.outstanding)}</strong>${icon("chevron", "row-chevron")}</button>`,
      )
