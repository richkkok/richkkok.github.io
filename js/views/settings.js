...
export function settingsView(state, cloud = {}) {
  ...
  return `<div class="page-intro compact-page-intro"><div><h1>설정</h1><p>자주 쓰는 것만 앞에 두고 나머지는 접어뒀어.</p></div><span class="version-tag">v${VERSION}</span></div>
    <div class="settings-simple-grid">
      ...
      <section class="card">${sectionTitle("자동 정리", "리치콕이 최대한 알아서 분류하고 계산해")}${row("거래파일 가져오기", "PDF · CSV · XLS · XLSX 자동 인식", "go-import", "upload")}${row("고정비 · 반복결제", `${state.recurring.length}개 등록${state.recurring.some((r) => r.autoDetected) ? ` · 자동 감지 ${state.recurring.filter((r) => r.autoDetected).length}개` : ""} · 실제 결제와 자동 연결`, "recurring-list", "repeat")}${row("초기 3개월 분석", s.baseline?.confirmed ? "분석자료 확인 완료" : "기간을 확인하면 추천 정확도가 올라가", "baseline-settings", "chart")}${row("자동분류 규칙", `${state.rules.length}개 학습됨 · 수정하면 다음부터 기억`, "rules-list", "filter")}</section>
    </div>
